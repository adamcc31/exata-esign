...
// GET /api/clients/[id]/pdf — Download signed PDF via presigned URL
export async function GET(req: NextRequest, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const result = await query(
      `SELECT signed_pdf_url, status, full_name, TO_CHAR(birth_date, 'DD-MM-YYYY') as birth_date FROM clients WHERE id = $1`,
      [id]
    );
