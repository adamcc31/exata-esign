...
export async function POST(req: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    if (!finalEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(finalEmail)) {
      return NextResponse.json({ error: 'Email wajib diisi dan harus valid (contoh: nama@domain.com).' }, { status: 400 });
    }
    ...
    try {
      ...
      const mailOptions = {
        ...
        text: `Yth, Bapak/Ibu ${client.full_name}\n\nBerikut kami lampirkan dokumen Surat Pernyataan Persetujuan untuk Layanan Speed Nenkin 20% dari Exata Indonesia.\n\nDokumen PDF ini berisi rincian kesepakatan mengenai proses pencairan dana pensiun Jepang (Nenkin) yang sudah Bapak setujui. Kami sarankan Bapak untuk mengunduh (download) dan menyimpan file ini sebagai pegangan ke depannya.\n\nJika ada informasi yang kurang jelas atau butuh bantuan lebih lanjut, jangan ragu untuk bertanya ke nomor whatsapp di 0811-9989-6308 (Exata Indonesia).\n\nTerima kasih atas kepercayaannya menggunakan layanan kami.\n\nSalam hangat,\n\nTim Exata Indonesia\nPT Sumber Rezeki Exata Indonesia\nGrand Galaxy City, RRG 5 no.9, Jaka Setia\nBekasi Selatan 17148, Indonesia\nWhatsApp: 0811-9989-6308`,
        attachments: [
          {
